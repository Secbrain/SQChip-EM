# -*- coding: utf-8 -*-
"""4-qubit dataset generator (qiskit-metal + pyEPR).

Layout: Square arrangement
    Q1 --- Q2
    |       |
    Q4 --- Q3

Bus couplers: Q1-Q2, Q2-Q3, Q3-Q4, Q4-Q1 (ring topology).
...
# -------------------------
@dataclass
class Layout4QParams:
    """4Q layout settings."""

    shared: base.LayoutParams = base.LayoutParams()
...
def _add_bus(design, name: str, q_start: str, pin_start: str, q_end: str, pin_end: str, p4: Layout4QParams):
    """Add a bus coupler between two qubit pads."""
    base.RouteMeander(
        design,
        name,
        options=base.Dict(
            total_length=p4.bus_total_length,
            fillet=p4.bus_fillet,
            trace_width=p4.bus_width,
            trace_gap=p4.bus_gap,
            meander=base.Dict(spacing=p4.bus_spacing),
            lead=base.Dict(start_straight=p4.bus_lead_start, end_straight=p4.bus_lead_end),
            pin_inputs=base.Dict(
...
def populate_design_4qubit(
    design,
    *,
    spacing_mm: float = 3.0,
    p4: Layout4QParams,
    lj_vars: Tuple[str, str, str, str] = ("Lj1", "Lj2", "Lj3", "Lj4"),
    cj_vars: Tuple[str, str, str, str] = ("Cj1", "Cj2", "Cj3", "Cj4"),
) -> None:
    """Populate planar design with 4 qubits in a square.
    ...
    Bus connections: Q1-Q2 (top), Q2-Q3 (right), Q3-Q4 (bottom), Q4-Q1 (left)

    Readout pads point outward from the square:
    ...
    # Positions: square centered at origin
    positions = {
        "Q1": (-half, +half),   # top-left
        "Q2": (+half, +half),   # top-right
        "Q3": (+half, -half),   # bottom-right
        "Q4": (-half, -half),   # bottom-left
    }

    # Q1: top-left corner -> readout to top-left, bus right (to Q2) and bus bottom (to Q4)
    _make_transmon(
        design, "Q1", x_mm=positions["Q1"][0], y_mm=positions["Q1"][1],
        p=p, lj_var=lj_vars[0], cj_var=cj_vars[0],
        bus_pads=[("bus_right", +1, +1), ("bus_down", -1, -1)],
        readout_loc_W=-1, readout_loc_H=+1,
    )

    # Q2: top-right corner -> readout to top-right, bus left (to Q1) and bus bottom (to Q3)
    _make_transmon(
        design, "Q2", x_mm=positions["Q2"][0], y_mm=positions["Q2"][1],
        p=p, lj_var=lj_vars[1], cj_var=cj_vars[1],
        bus_pads=[("bus_left", -1, +1), ("bus_down", +1, -1)],
        readout_loc_W=+1, readout_loc_H=+1,
    )

    # Q3: bottom-right corner -> readout to bottom-right, bus top (to Q2) and bus left (to Q4)
    _make_transmon(
        design, "Q3", x_mm=positions["Q3"][0], y_mm=positions["Q3"][1],
        p=p, lj_var=lj_vars[2], cj_var=cj_vars[2],
        bus_pads=[("bus_top", +1, +1), ("bus_left", -1, -1)],
        readout_loc_W=+1, readout_loc_H=-1,
    )

    # Q4: bottom-left corner -> readout to bottom-left, bus top (to Q1) and bus right (to Q3)
    _make_transmon(
        design, "Q4", x_mm=positions["Q4"][0], y_mm=positions["Q4"][1],
        p=p, lj_var=lj_vars[3], cj_var=cj_vars[3],
        bus_pads=[("bus_top", -1, +1), ("bus_right", +1, -1)],
        readout_loc_W=-1, readout_loc_H=-1,
    )
    ...
    # Bus couplers (ring topology)
    _add_bus(design, "Bus_12", "Q1", "bus_right", "Q2", "bus_left", p4)    # top edge
    _add_bus(design, "Bus_23", "Q2", "bus_down",  "Q3", "bus_top",  p4)    # right edge
    _add_bus(design, "Bus_34", "Q3", "bus_left",  "Q4", "bus_right", p4)   # bottom edge
    _add_bus(design, "Bus_41", "Q4", "bus_top",   "Q1", "bus_down",  p4)   # left edge
...
def build_chip_summary_4q(*, sample_id: str) -> TDict[str, Any]:
    chip = {}
...
def postprocess_4q(payload: TDict[str, Any]) -> TDict[str, Any]:
...
def run_analysis_pipeline_4q(
    ...
    *,
    ...
    do_hfss: bool,
    do_q3d: bool,
    lj_nh: Tuple[float, float, float, float],
    cj_fF_user: Tuple[float, float, float, float],
    ...
    payload = build_chip_summary_4q(sample_id=sample_id)
    ...
    if do_hfss:
        for attempt in [1, 2]:
            ...
            try:
                ...
                target = workdir / f"EIG4Q_{sim_tag}_a{attempt}.aedt"
                ...
                eig.sim.setup.n_modes = 2 * NUM_Q + 4  # extra headroom
                eig.sim.setup.max_passes = 12
                ...
                eig.sim.run(
                    name=f"Eig4Q_{sim_tag}_a{attempt}",
                ...
                pairs = [(0, 1, "chi12_MHz"), (1, 2, "chi23_MHz"), (2, 3, "chi34_MHz"),
                         (3, 0, "chi41_MHz"), (0, 2, "chi13_MHz"), (1, 3, "chi24_MHz")]
                for a, b, key in pairs:
                    payload["chip"][key] = float(chi_matrix[idx_q[a], idx_q[b]])
    ...
    if do_q3d:
        for attempt in [1, 2]:
            ...
            try:
                ...
                workdir = base._ansys_short_workdir(root)
                target = workdir / f"Q3D4Q_{sim_tag}_a{attempt}.aedt"
                ...
                lom.sim.run(name=f"Q3D4Q_{sim_tag}_a{attempt}", components=list(design.components.keys()))
    ...
    payload = postprocess_4q(payload)
...
if __name__ == "__main__":
    ...
    lp_shared = base.LayoutParams(
        ...
        feed_fillet="25um",
        ro_fillet="25um",
        # Keep RouteStraight leads short; long leads can self-overlap and fail in Ansys.
        ro_lead_start="80um",
        ro_lead_end="80um",
    ...
    bus_length_mm = max(5.0, QUBIT_SPACING_MM + 2.0)
    ...
    sample_id = (
        "chip_4q_square_"
        f"spacing{base.fmt_mm_id(float(QUBIT_SPACING_MM))}mm"
    ...
    try:
        ...
        populate_design_4qubit(
            design,
            spacing_mm=QUBIT_SPACING_MM,
            p4=p4,
        )

        result = run_analysis_pipeline_4q(
            design,
            sample_id,
            ...
            lj_nh=LJ_NH,
            cj_fF_user=CJ_FF,
            inputs_sweep={"spacing_mm": float(QUBIT_SPACING_MM)},
            inputs_qubits={
                "Q1": {"x_mm": float(-QUBIT_SPACING_MM / 2.0), "y_mm": float(+QUBIT_SPACING_MM / 2.0)},
                "Q2": {"x_mm": float(+QUBIT_SPACING_MM / 2.0), "y_mm": float(+QUBIT_SPACING_MM / 2.0)},
                "Q3": {"x_mm": float(+QUBIT_SPACING_MM / 2.0), "y_mm": float(-QUBIT_SPACING_MM / 2.0)},
                "Q4": {"x_mm": float(-QUBIT_SPACING_MM / 2.0), "y_mm": float(-QUBIT_SPACING_MM / 2.0)},
            },
        )

        print(f"[Done] 4Q completed, status={result.get('status')}")

    except Exception as e:
        print(f"[ERROR] 4Q failed: {e}")
    finally:
        base._mpl_cleanup()
