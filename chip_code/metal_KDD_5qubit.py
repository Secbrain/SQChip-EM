# -*- coding: utf-8 -*-
"""5-qubit dataset generator (qiskit-metal + pyEPR).

Layout: T-shape topology
        Q1 === Q2 === Q3
               鈺?
               Q4
               鈺?
...
Bus couplers (adjacent only): Q1-Q2, Q2-Q3, Q2-Q4, Q4-Q5
...
def _add_bus(
    ...
    *,
    ...
    meander = base.Dict(spacing=p5.bus_spacing)
    ...
    base.RouteMeander(
        ...
        name,
        options=base.Dict(
            total_length=p5.bus_total_length,
            fillet=p5.bus_fillet,
            trace_width=p5.bus_width,
            trace_gap=p5.bus_gap,
            meander=meander,
            lead=base.Dict(start_straight=p5.bus_lead_start, end_straight=p5.bus_lead_end),
            pin_inputs=base.Dict(
...
def populate_design_5qubit(
    design,
    *,
    dx_h_mm: float = 3.0,
    dy_v_mm: float = 3.0,
    p5: Layout5QParams,
    lj_vars: Tuple[str, str, str, str, str] = ("Lj1", "Lj2", "Lj3", "Lj4", "Lj5"),
    cj_vars: Tuple[str, str, str, str, str] = ("Cj1", "Cj2", "Cj3", "Cj4", "Cj5"),
) -> None:
    """Populate planar design with 5 qubits in a T-shape pattern.

    Layout:
        Q1 === Q2 === Q3    (horizontal bar, y = +dy_v)
               鈺?
               Q4           (y = 0)
               鈺?
               Q5           (y = -dy_v)

    Q2 is at the T-junction center.

    Bus couplers (adjacent only, 4 total):
        - Horizontal: Q1-Q2, Q2-Q3
        - Vertical: Q2-Q4, Q4-Q5

    Readout pads:
        Q1: top-left (away from Q2)
        Q2: top (away from Q4)
        Q3: top-right (away from Q2)
        Q4: right (away from Q2 and Q5)
        Q5: bottom (away from Q4)
    """
    ...
    # Positions: T-shape centered at origin
    # Q2 is at the T-junction (center of horizontal bar)
    # Horizontal bar at y = +dy_v
    # Vertical stem goes down from Q2
    positions = {
        "Q1": (-dx_h_mm, +dy_v_mm),   # left of T-bar
        "Q2": (0.0, +dy_v_mm),        # T-junction center
        "Q3": (+dx_h_mm, +dy_v_mm),   # right of T-bar
        "Q4": (0.0, 0.0),             # middle of stem
        "Q5": (0.0, -dy_v_mm),        # bottom of stem
    }

    # Q1 (left of T-bar): readout top-left, bus pad: right (to Q2)
    _make_transmon(
    ...
    # Q2 (T-junction center): readout top, bus pads: left (Q1), right (Q3), down (Q4)
    _make_transmon(
        ...
        bus_pads=[
            ...
            #   - Q2 right-bottom -> Q4 right-top (vertical stem)
            #   - Q2 right-top    -> Q3 left-bottom (top bar)
            ("bus_right", +1, +1),  # toward Q3
            ("bus_down", +1, -1),   # toward Q4
        ],
        readout_loc_W=0, readout_loc_H=+1,
    )

    # Q3 (right of T-bar): readout top-right, bus pad: left (to Q2)
    _make_transmon(
    ...
    # Q4 (middle of stem): readout right, bus pads: up (Q2), down (Q5)
    _make_transmon(
        design, "Q4", x_mm=positions["Q4"][0], y_mm=positions["Q4"][1],
        p=p, lj_var=lj_vars[3], cj_var=cj_vars[3],
        bus_pads=[
            ("bus_up", +1, +1),     # toward Q2
            ("bus_down", -1, -1),   # toward Q5
        ],
        # Put readout on the lower-right pad so the upper-right pad is free
        # for the vertical bus connection (cleaner interface).
        readout_loc_W=+1, readout_loc_H=-1,
    )

    # Q5 (bottom of stem): readout bottom, bus pad: up (to Q4)
    _make_transmon(
    ...
    # Readout chains (align tee to readout pad normal).
    tee_offset = 0.45
    tee_extra_norm_mm = {
        # Q4 sits near the vertical bus stem; keep a little clearance,
        # but keep RO_RES4 short (avoid the long "handle").
        "Q4": 0.25,
    }
    for i in range(1, 6):
        q = f"Q{i}"
        px, py, nx, ny, _tx, _ty = _get_pin_frame(design, q, "readout")
        ori = _tee_orientation_from_readout_normal(nx, ny)
        extra = float(tee_extra_norm_mm.get(q, 0.0) or 0.0)
        cx = px + nx * (tee_offset + extra)
        cy = py + ny * (tee_offset + extra)
        lp_dx = _clamp_lp_dx_along_normal(coupler_x_mm=cx, coupler_y_mm=cy, nx=nx, ny=ny, p=p)
        lp_x = cx + nx * lp_dx
        lp_y = cy + ny * lp_dx
    ...
    # Bus couplers (adjacent only, 4 total)
    # Horizontal bar
    _add_bus(design, "Bus_12", "Q1", "bus_right", "Q2", "bus_left", p5)
    _add_bus(design, "Bus_23", "Q2", "bus_right", "Q3", "bus_left", p5)
    # Vertical stem
    _add_bus(design, "Bus_24", "Q2", "bus_down", "Q4", "bus_up", p5, asymmetry="-400um")
    _add_bus(design, "Bus_45", "Q4", "bus_down", "Q5", "bus_up", p5)
...
def build_chip_summary_5q(*, sample_id: str) -> TDict[str, Any]:
    chip = {}
    ...
    chip.update(
        {
            # Adjacent couplings (T-shape)
            "chi12_MHz": None,  # Q1-Q2 horizontal
            "chi23_MHz": None,  # Q2-Q3 horizontal
            "chi24_MHz": None,  # Q2-Q4 vertical (T-junction)
            "chi45_MHz": None,  # Q4-Q5 vertical
            # Non-adjacent couplings (for reference)
            "chi13_MHz": None,  # Q1-Q3 (across T-bar)
            "chi14_MHz": None,  # Q1-Q4
            "chi15_MHz": None,  # Q1-Q5
            "chi34_MHz": None,  # Q3-Q4
            "chi35_MHz": None,  # Q3-Q5
            "chi25_MHz": None,  # Q2-Q5
        }
    )
...
def postprocess_5q(payload: TDict[str, Any]) -> TDict[str, Any]:
...
def run_analysis_pipeline_5q(
    ...
    *,
    ...
    do_hfss: bool,
    do_q3d: bool,
    lj_nh: Tuple[float, float, float, float, float],
    cj_fF_user: Tuple[float, float, float, float, float],
    ...
    payload = build_chip_summary_5q(sample_id=sample_id)
    ...
    if do_hfss:
        for attempt in [1, 2]:
            ...
            try:
                ...
                target = workdir / f"EIG5Q_{sim_tag}_a{attempt}.aedt"
                ...
                # Numerical diagonalization is disabled below; we only need enough modes
                # to cover NUM_Q qubits + NUM_Q readouts.
                eig.sim.setup.n_modes = 2 * NUM_Q + 2
                ...
                eig.sim.run(
                    name=f"Eig5Q_{sim_tag}_a{attempt}",
                ...
                # idx: Q1=0, Q2=1, Q3=2, Q4=3, Q5=4
                # Adjacent (T-shape)
                payload["chip"]["chi12_MHz"] = float(chi_matrix[idx_q[0], idx_q[1]])
                payload["chip"]["chi23_MHz"] = float(chi_matrix[idx_q[1], idx_q[2]])
                payload["chip"]["chi24_MHz"] = float(chi_matrix[idx_q[1], idx_q[3]])
                payload["chip"]["chi45_MHz"] = float(chi_matrix[idx_q[3], idx_q[4]])
                # Non-adjacent (for reference)
                payload["chip"]["chi13_MHz"] = float(chi_matrix[idx_q[0], idx_q[2]])
                payload["chip"]["chi14_MHz"] = float(chi_matrix[idx_q[0], idx_q[3]])
                payload["chip"]["chi15_MHz"] = float(chi_matrix[idx_q[0], idx_q[4]])
                payload["chip"]["chi34_MHz"] = float(chi_matrix[idx_q[2], idx_q[3]])
                payload["chip"]["chi35_MHz"] = float(chi_matrix[idx_q[2], idx_q[4]])
    ...
    if do_q3d:
        for attempt in [1, 2]:
            ...
            try:
                ...
                workdir = base._ansys_short_workdir(root)
                target = workdir / f"Q3D5Q_{sim_tag}_a{attempt}.aedt"
                ...
                lom.sim.run(name=f"Q3D5Q_{sim_tag}_a{attempt}", components=list(design.components.keys()))
    ...
    payload = postprocess_5q(payload)
...
if __name__ == "__main__":
    dataset_root = Path("./data/sqchip_em_5q")
    ...
    # Sweep parameters: horizontal and vertical spacing
    ...
    csv_headers = [
        "sample_id",
        "dx_h_mm", "dy_v_mm",
        "fq1_GHz", "fq2_GHz", "fq3_GHz", "fq4_GHz", "fq5_GHz",
        "fr1_GHz", "fr2_GHz", "fr3_GHz", "fr4_GHz", "fr5_GHz",
        "chi12_MHz", "chi23_MHz", "chi24_MHz", "chi45_MHz",
        "status", "error",
    ]

    summary_csv_path = dataset_root / "summary_5q.csv"
    ...
    for dx_h, dy_v in product(DX_H_LIST, DY_V_LIST):
        ...
        bus_length_mm = max(5.0, max(dx_h, dy_v) + 2.0)
        ...
        try:
            ...
            result = run_analysis_pipeline_5q(
                design,
                sample_id,
                ...
                inputs_qubits={
                    "Q1": {"x_mm": float(-dx_h), "y_mm": float(+dy_v)},
                    "Q2": {"x_mm": float(0.0), "y_mm": float(+dy_v)},
                    "Q3": {"x_mm": float(+dx_h), "y_mm": float(+dy_v)},
                    "Q4": {"x_mm": float(0.0), "y_mm": float(0.0)},
                    "Q5": {"x_mm": float(0.0), "y_mm": float(-dy_v)},
                },
            )
            ...
            out = dict(
                ...
                chi24_MHz=chip.get("chi24_MHz"),
                chi45_MHz=chip.get("chi45_MHz"),
    ...
    print(f"\n[Done] 5Q T-shape dataset generation completed. Total samples: {sample_count}")
