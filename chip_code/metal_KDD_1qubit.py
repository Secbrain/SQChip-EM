...
# 鍗曢€氶亾鐪熷疄 Qi 浠跨湡鍙傛暟
# -------------------------
# Qi 鐨勮绠楅€昏緫锟?#   p_bulk = U_E(main) / U_E(AllObjects)   锟?锟?HFSS Fields Calculator 绉垎锛岀湡瀹炰豢锟?#   Qi     = 1 / (p_bulk * TAN_DELTA_BULK) 锟?涔樹笂浜哄伐璁惧畾鐨勬潗鏂欐崯锟?#
# TAN_DELTA_BULK 鏄敮涓€鐨勪汉涓鸿瀹氶噺锛屼唬琛ㄤ綘鍋囪鐨勮‖搴曠患鍚堜粙璐ㄦ崯鑰楋紙鍚姌鍚堣繘鏉ョ殑鐣岄潰鎹熻€楋級锟?# 鍙傝€冨€硷細Al/Si 鏍囧噯 fab ~1e-5锛涘ソ fab Al/Si ~3e-6锛汿a/sapphire SOTA ~1e-6
# 閫氳繃鐜鍙橀噺瑕嗙洊锛歁ETAL_TAN_DELTA_BULK=3e-6 python metal_KDD_1qubit.py
TAN_DELTA_BULK = float(os.environ.get("METAL_TAN_DELTA_BULK", "1e-5"))
...
def build_chip_summary(*, sample_id: str) -> TDict[str, Any]:
    return {
        "meta": {
            ...
            "loss_model": {"type": "single_channel_bulk", "tan_delta_bulk": None},
        },
        "resonators": {
            "readout": {
                "f_GHz": None,
...
def pick_cin_prefer_tee_bodies(Cmat_fF: pd.DataFrame, tee_name: str = "RO_TEE") -> TDict[str, Any]:
    """
    鎻愬彇璇诲嚭鑵旂殑澶栭儴鑰﹀悎鐢靛 Cin锟?
    鐗╃悊鍚箟锛欳in 锟?qubit readout pad 锟?tee 鐨勮€﹀悎鐢靛锟?    鐢ㄤ簬璁＄畻 kappa_e锛坙ambda/4 鍏紡锛夛拷?
    鑺傜偣浼樺厛绾э紙鐢辩墿鐞嗗惈涔夊喅瀹氾級锟?    1. pad_top_Q1 <-> cap_body_0锛歲ubit pad 锟?tee 璋愭尟渚э紙姝ｇ‘锟?Cin锛寏30-60 fF锟?    2. pad_top_Q1 <-> cap_body_1锛歲ubit pad 锟?tee 棣堢嚎锟?    3. cap_body_0 涓庢渶寮虹殑锟?cap_body_1銆侀潪鑷韩鑺傜偣
    4. 鏈€鍚庢墠鑰冭檻 cap_body_0 <-> cap_body_1
       锛堣繖锟?tee 鍐呴儴鎸囬棿鐢靛 ~900-1200 fF锛屼笉鏄湡姝ｇ殑 Cin锛屼笉搴旂敤锟?kappa_e锟?    """
    nodes = list(Cmat_fF.index)
    cap0 = f"cap_body_0_{tee_name}"
    cap1 = f"cap_body_1_{tee_name}"
    pad_top = "pad_top_Q1"

    # 浼樺厛锟?1锛歱ad_top_Q1 <-> cap_body_0锛坬ubit 锟?tee 璋愭尟渚э級
    if pad_top in nodes and cap0 in nodes:
        cin = abs(float(Cmat_fF.loc[pad_top, cap0]))
        if cin > 0:
            return {"Cin_fF": float(cin), "res_node": pad_top, "feed_node": cap0,
                    "pair": f"{pad_top} <-> {cap0}"}

    # 浼樺厛锟?2锛歱ad_top_Q1 <-> cap_body_1锛坬ubit 锟?tee 棣堢嚎渚э級
    if pad_top in nodes and cap1 in nodes:
        cin = abs(float(Cmat_fF.loc[pad_top, cap1]))
        if cin > 0:
            return {"Cin_fF": float(cin), "res_node": pad_top, "feed_node": cap1,
                    "pair": f"{pad_top} <-> {cap1} (feedline side)"}

    # 浼樺厛锟?3锛歝ap_body_0 涓庢渶寮虹殑锟?cap_body_1銆侀潪鑷韩鑺傜偣
    if cap0 in nodes:
        row = Cmat_fF.loc[cap0].copy()
        exclude = [n for n in [cap0, cap1] if n in row.index]
        row = row.drop(labels=exclude, errors="ignore")
        if len(row.index) > 0:
            j = str(row.abs().idxmax())
            cin = abs(float(Cmat_fF.loc[cap0, j]))
            if cin > 0:
                return {"Cin_fF": float(cin), "res_node": j, "feed_node": cap0,
                        "pair": f"{cap0} <-> {j} (fallback)"}

    # 浼樺厛锟?4锛堟渶锟?fallback锛夛細cap_body_0 <-> cap_body_1
    # 杩欐槸 tee 鍐呴儴鐢靛锛岄€氬父涓嶅锛屼絾淇濈暀浣滀负鍏滃簳
    if cap0 in nodes and cap1 in nodes:
        cin = abs(float(Cmat_fF.loc[cap0, cap1]))
        return {"Cin_fF": float(cin), "res_node": cap0, "feed_node": cap1,
                "pair": f"{cap0} <-> {cap1} (internal-tee-fallback, verify layout)"}
...
class LayoutParams:
    ...
    cap_gap: str = "22um"
    cap_width: str = "8um"
    finger_length: str = "19um"
    finger_count: str = "2"
    cap_distance: str = "220um"
...
def populate_design(design, q_x_mm: float, q_y_mm: float, *, p: LayoutParams) -> None:
    ...
    # Qi 锟?HFSS 鍦虹Н锟?+ TAN_DELTA_BULK 鍦ㄥ悗澶勭悊涓绠楋紝
    # 涓嶉渶瑕佸湪 design.chips.main.material 閲屽啓 tan_delta锟?
    q1 = TransmonPocket(
        design,
        "Q1",
        options=dict(
    ...
    coupler_x = qx_pin + 1.0
    coupler_y = qy_pin + 2.0
...
def _ansys_ensure_renderer_ready(renderer, *, timeout_s: float = 15.0) -> None:
    """Ensure the renderer is connected exactly once.

    Normal path: EPRanalysis/LOManalysis construction already connects to Ansys,
    so we should NOT call renderer.start() again. Only fall back to start() when
    the renderer is not ready after construction.
    """
    if _ansys_wait_ready(renderer, timeout_s=2.0, interval_s=0.2):
        return
    ...
    if not _ansys_wait_ready(renderer, timeout_s=timeout_s, interval_s=0.2):
        raise RuntimeError("Ansys renderer is not ready after initialization/start.")
...
def _ansys_prepare_project(renderer, target_aedt: Path) -> None:
    target_aedt.parent.mkdir(parents=True, exist_ok=True)
    _ansys_ensure_renderer_ready(renderer, timeout_s=15.0)
    ...
    if not prepared:
        try:
            if hasattr(renderer, "new_ansys_project"):
                renderer.new_ansys_project()
            elif hasattr(renderer, "rdesktop") and renderer.rdesktop:
...
def run_analysis_pipeline(
    ...
    *,
    ...
    payload["meta"]["loss_model"]["tan_delta_bulk"] = TAN_DELTA_BULK
    ...
    if do_hfss:
        for attempt in [1, 2]:
            ...
            try:
                ...
                step = "hfss.ensure_ready"
                _ansys_ensure_renderer_ready(hfss, timeout_s=15.0)
                ...
                # dissipatives 鍙仛妯″紡鏍囪鐢紙EPR 鍐呴儴鐢級锟?                # Qi 鐢卞悗闈㈢殑 calc_energy_electric() 鐪熷疄绉垎绠楋紝涓嶄緷锟?tan_delta 璁剧疆锟?                eig.setup.dissipatives = {"dielectrics_bulk": ["main"]}
                ...
                f_q = float(freqs[idx_q])
                res_window_min = f_q + 0.3   # 鏈€锟?detuning 300 MHz
                res_window_max = f_q + 3.5   # 鏈€锟?detuning 3.5 GHz
                ...
                for i, f in enumerate(freqs):
                    if not (res_window_min <= float(f) <= res_window_max):
                        row[i] = -1.0
                ...
                # =========================================================
                # 锟?鐪熷疄鍗曢€氶亾 Qi 浠跨湡
                # Qi = 1 / (p_bulk * TAN_DELTA_BULK)
                # p_bulk = U_E(main) / U_E(AllObjects)  锟?HFSS Fields Calculator 鐪熷疄绉垎
                # 涓嶄娇鐢ㄤ换锟?鍙嶆帹"锟?甯告暟 fallback"閫昏緫锟?                # =========================================================
                import pyEPR.core_distributed_analysis as _cda_mod
                eprd = _cda_mod.DistributedAnalysis(pinfo)

                # variation '0' 瀵瑰簲褰撳墠杩欐 eigenmode 锟?                _variation = "0"

                # ---------- 璇诲嚭锟?Qi ----------
                Qi_val = None
                p_bulk_res = None
                try:
                    U_E_total = eprd.calc_energy_electric(_variation, obj="AllObjects")
                    U_E_bulk  = eprd.calc_energy_electric(_variation, obj="main")
                    if U_E_total and U_E_total > 0 and U_E_bulk and U_E_bulk > 0:
                        p_bulk_res = float(U_E_bulk) / float(U_E_total)
                        if 0.05 <= p_bulk_res <= 0.9999:
                            Qi_val = 1.0 / (p_bulk_res * TAN_DELTA_BULK)
                            ro["warnings"].append(
                                f"Qi (real HFSS integral): p_bulk={p_bulk_res:.4g} "
                                f"tan_delta_bulk={TAN_DELTA_BULK:.1e} Qi={Qi_val:.4g}"
                            )
                            print(
                                f"[EPR] Qi(res): p_bulk={p_bulk_res:.4g} "
                                f"tan_delta={TAN_DELTA_BULK:.1e} => Qi={Qi_val:.4g}",
                                flush=True,
                            )
                        else:
                            ro["warnings"].append(
                                f"p_bulk_res={p_bulk_res:.4g} out of range [0.05,1); "
                                "check HFSS substrate object name 'main'."
                            )
                except Exception as _e_qi:
                    ro["warnings"].append(f"calc_energy_electric failed: {_safe_ascii(_e_qi)}")
                    print(f"[WARN] [EPR] calc_energy_electric error: {_safe_ascii(_e_qi)}", flush=True)

                # p_bulk 瀛樺叆 JSON锛堣瘖鏂敤锟?                ro["p_bulk"]             = p_bulk_res
                ro["tan_delta_bulk"]     = TAN_DELTA_BULK
                ro["Qi"]                 = float(Qi_val) if Qi_val is not None else None
                ro["kappa_i_over_2pi_Hz"] = (float(f_res) * 1e9) / Qi_val if Qi_val else None

                # ---------- Qubit Q_dielectric锛堝悓涓€娆＄Н鍒嗙粨鏋滐紝鐩存帴澶嶇敤锟?----------
                if p_bulk_res is not None and 0.05 <= p_bulk_res <= 0.9999:
                    q1["Q_dielectric_main"] = 1.0 / (p_bulk_res * TAN_DELTA_BULK)
    ...
    if do_q3d:
        for attempt in [1, 2]:
            ...
            try:
                ...
                step = "q3d.ensure_ready"
                _ansys_ensure_renderer_ready(q3d, timeout_s=15.0)
...
if __name__ == "__main__":
    ...
    TEE_FINGER_LENGTH_UM = 20
    TEE_FINGER_COUNT = 2
    TEE_CAP_GAP_UM = 6.0
    TEE_CAP_WIDTH_UM = 10.0
    TEE_CAP_DISTANCE_UM = 120.0

    RO_L_MM = 8.0
    Q_RO_PAD_GAP_UM = 6.0   # 鍘熸潵 1.4um 澶皬锛実ap 灏忎細璁╄€﹀悎鐖嗙偢锛岀洰锟?8-15um

    TARGET_ABS_CHI_MHZ = 2.0
    MAX_SAMPLES = 1  # None means no limit
    ...
    csv_headers = [
        ...
        "p_bulk","tan_delta_bulk","kappa_i_over_kappa_e",
    ...
    lp = LayoutParams(
        ...
        # 锟?澧炲ぇ readout pad锛堟洿寮虹數瀹硅€﹀悎锛実 寰€涓婅蛋锟?        ro_pad_w="180um",
        ro_pad_h="150um",
        ...
        try:
            ...
            out.update(
                dict(
                    ...
                    p_bulk=ro.get("p_bulk"),
                    tan_delta_bulk=ro.get("tan_delta_bulk"),
