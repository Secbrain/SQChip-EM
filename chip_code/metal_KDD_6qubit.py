# -*- coding: utf-8 -*-
"""6-qubit dataset generator (qiskit-metal + pyEPR).
...
Bus couplers:
    Horizontal: Q1-Q2, Q2-Q3, Q4-Q5, Q5-Q6
    Vertical: Q1-Q4, Q2-Q5, Q3-Q6
...
def _add_bus(
    ...
    *,
    ...
    meander = base.Dict(spacing=p6.bus_spacing)
    ...
    base.RouteMeander(
        ...
        name,
        options=base.Dict(
            total_length=p6.bus_total_length,
            fillet=p6.bus_fillet,
            trace_width=p6.bus_width,
            trace_gap=p6.bus_gap,
            meander=meander,
            lead=base.Dict(start_straight=p6.bus_lead_start, end_straight=p6.bus_lead_end),
            pin_inputs=base.Dict(
...
def populate_design_6qubit(
    design,
    *,
    dx_h_mm: float = 3.0,
    dy_v_mm: float = 3.0,
    p6: Layout6QParams,
    lj_vars: Tuple[str, ...] = ("Lj1", "Lj2", "Lj3", "Lj4", "Lj5", "Lj6"),
    cj_vars: Tuple[str, ...] = ("Cj1", "Cj2", "Cj3", "Cj4", "Cj5", "Cj6"),
) -> None:
    """Populate planar design with 6 qubits in a 2x3 grid.

    Layout:
        Q1 --- Q2 --- Q3   (top row, y = +dy_v/2)
        |      |      |
        Q4 --- Q5 --- Q6   (bottom row, y = -dy_v/2)
    ...
    Readout pads:
        Q1: top-left, Q2: top, Q3: top-right
        Q4: bottom-left, Q5: bottom, Q6: bottom-right
    """
    ...
    # Positions: 2x3 grid centered at origin
    # Top row: Q1, Q2, Q3 at y = +half_dy
    # Bottom row: Q4, Q5, Q6 at y = -half_dy
    positions = {
        "Q1": (-dx_h_mm, +half_dy),
        "Q2": (0.0, +half_dy),
        "Q3": (+dx_h_mm, +half_dy),
    ...
    # Q1 (top-left): readout top-left, bus pads: right (to Q2), down (to Q4)
    _make_transmon(
    ...
    # Q2 (top-center): readout top, bus pads: left (to Q1), right (to Q3), down (to Q5)
    _make_transmon(
    ...
    for i in range(1, 7):
        ...
        ori_i = int(float(ori)) % 360
        if ori_i in (0, 180):
            # Horizontal tees: pick the prime pin on the LP side.
            prime_pin_override = "prime_start" if float(cx) < 0 else "prime_end"
        elif ori_i == 270 and q == "Q3":
            # Q3 tee points right (ori=270). For this case, the LP lands at the same Y as the tee,
            # which makes the default tie-break connect to the lower prime pin.
            # Force RO_FEED3 to connect to the upper cap-body interface (prime_start).
            prime_pin_override = "prime_start"
    ...
    # Bus couplers
    # Horizontal (top row)
    _add_bus(design, "Bus_12", "Q1", "bus_right", "Q2", "bus_left", p6)
    _add_bus(design, "Bus_23", "Q2", "bus_right", "Q3", "bus_left", p6)
    # Horizontal (bottom row)
    _add_bus(design, "Bus_45", "Q4", "bus_right", "Q5", "bus_left", p6)
    _add_bus(design, "Bus_56", "Q5", "bus_right", "Q6", "bus_left", p6)
    # Vertical
    _add_bus(design, "Bus_14", "Q1", "bus_down", "Q4", "bus_up", p6)
    _add_bus(design, "Bus_25", "Q2", "bus_down", "Q5", "bus_up", p6)
    _add_bus(design, "Bus_36", "Q3", "bus_down", "Q6", "bus_up", p6)
...
def build_chip_summary_6q(*, sample_id: str) -> TDict[str, Any]:
    chip = {}
...
def postprocess_6q(payload: TDict[str, Any]) -> TDict[str, Any]:
...
def run_analysis_pipeline_6q(
    ...
    *,
    ...
    payload = build_chip_summary_6q(sample_id=sample_id)
    ...
    if do_hfss:
        for attempt in [1, 2]:
            ...
            try:
                ...
                target = workdir / f"EIG6Q_{sim_tag}_a{attempt}.aedt"
                ...
                eig.sim.run(
                    name=f"Eig6Q_{sim_tag}_a{attempt}",
                ...
                # idx: Q1=0, Q2=1, Q3=2, Q4=3, Q5=4, Q6=5
                # Horizontal (top row)
    ...
    if do_q3d:
        for attempt in [1, 2]:
            ...
            try:
                ...
                workdir = base._ansys_short_workdir(root)
                target = workdir / f"Q3D6Q_{sim_tag}_a{attempt}.aedt"
                ...
                lom.sim.run(name=f"Q3D6Q_{sim_tag}_a{attempt}", components=list(design.components.keys()))
    ...
    payload = postprocess_6q(payload)
...
if __name__ == "__main__":
    dataset_root = Path("./data/sqchip_em_6q")
    ...
    csv_headers = [
        "sample_id",
        "dx_h_mm", "dy_v_mm",
        "fq1_GHz", "fq2_GHz", "fq3_GHz", "fq4_GHz", "fq5_GHz", "fq6_GHz",
        "fr1_GHz", "fr2_GHz", "fr3_GHz", "fr4_GHz", "fr5_GHz", "fr6_GHz",
        "chi12_MHz", "chi23_MHz", "chi45_MHz", "chi56_MHz",
        "chi14_MHz", "chi25_MHz", "chi36_MHz",
        "status", "error",
    ]

    summary_csv_path = dataset_root / "summary_6q.csv"
    ...
    for dx_h, dy_v in product(DX_H_LIST, DY_V_LIST):
        ...
        try:
            ...
            result = run_analysis_pipeline_6q(
                design,
                sample_id,
                ...
                inputs_qubits={
                    "Q1": {"x_mm": float(-dx_h), "y_mm": float(+dy_v / 2.0)},
                    "Q2": {"x_mm": float(0.0), "y_mm": float(+dy_v / 2.0)},
                    "Q3": {"x_mm": float(+dx_h), "y_mm": float(+dy_v / 2.0)},
                    "Q4": {"x_mm": float(-dx_h), "y_mm": float(-dy_v / 2.0)},
                    "Q5": {"x_mm": float(0.0), "y_mm": float(-dy_v / 2.0)},
                    "Q6": {"x_mm": float(+dx_h), "y_mm": float(-dy_v / 2.0)},
                },
            )
            ...
            out = dict(
                ...
                chi14_MHz=chip.get("chi14_MHz"),
                chi25_MHz=chip.get("chi25_MHz"),
                chi36_MHz=chip.get("chi36_MHz"),
    ...
    print(f"\n[Done] 6Q dataset generation completed. Total samples: {sample_count}")
